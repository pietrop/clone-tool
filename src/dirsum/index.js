 ...
 *
 * originally from
 * https://github.com/mcavage/node-dirsum/blob/master/lib/dirsum.js
 * But modified to exclude .DS_Store local files from comparison, on line 78
 */
    ...
    files.forEach(function (f) {
      ...
      fs.stat(path, function (err, stats) {
        ...
        if (stats.isDirectory()) {
          ...
          // Modified to exclude .DS_Store local files from comparison
        } else if (stats.isFile() && f !== '.DS_Store') {
          ...
          console.info('Deliberately skipping hash of %s', f);
          if (++hashed > files.length) {
            return callback(undefined, _summarize(method, hashes));
          }
        } else {
